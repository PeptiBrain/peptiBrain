...
import { PublicReconstitutionCalculator, type CalcPrefill } from "@/components/app/calculator/PublicReconstitutionCalculator";
...
export default async function CalculadoraPage({
  ...
  const prefill: CalcPrefill = {
    vialAmount: one(sp.vial),
    vialUnit: one(sp.vialUnit),
    bacWater: one(sp.bac),
    doseAmount: one(sp.dose),
    doseUnit: one(sp.doseUnit),
  };
      ...
      <main id="main-content" className="flex-1">
        <div className="mx-auto max-w-2xl px-4 py-10 sm:py-14">
          <div className="flex items-center gap-2 text-sm font-medium text-primary">
            <Calculator className="size-4" aria-hidden /> {t("kicker")}
          </div>
          ...
          <div className="mt-6">
            <PublicReconstitutionCalculator prefill={prefill} />
          </div>
