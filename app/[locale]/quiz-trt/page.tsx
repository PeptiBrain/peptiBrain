...
import { TrtQuizTool } from "@/components/app/calculator/TrtQuizTool";
...
export async function generateMetadata({
  ...
  const t = await getTranslations({ locale, namespace: "TrtQuiz" });
...
export default async function TrtQuizPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
      ...
      <main id="main-content" className="flex-1">
        <div className="mx-auto max-w-3xl px-4 py-10 sm:py-14">
          <div className="flex items-center gap-2 text-sm font-medium text-primary">
            <ClipboardCheck className="size-4" aria-hidden /> {t("kicker")}
          </div>
          ...
          <ToolDisclaimer />
          <ToolCta />
          <ToolCrossLinks current="trtQuiz" />
        </div>
      </main>
      <Footer />
      <JsonLd data={jsonLd} />
