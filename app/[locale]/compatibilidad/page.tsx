...
import { CompatibilidadTool } from "@/components/app/calculator/CompatibilidadTool";
...
export default async function CompatibilidadPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
      ...
      <main id="main-content" className="flex-1">
        <div className="mx-auto max-w-3xl px-4 py-10 sm:py-14">
          <div className="flex items-center gap-2 text-sm font-medium text-primary">
            <Shuffle className="size-4" aria-hidden /> {t("kicker")}
          </div>
