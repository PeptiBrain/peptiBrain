...
export async function generateMetadata({
  ...
  const t = await getTranslations({ locale, namespace: "Comparador" });
...
export default async function ComparadorPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
    ...
    <>
      ...
      <main id="main-content" className="flex-1">
        <div className="mx-auto max-w-3xl px-4 py-10 sm:py-14">
          ...
          <ToolDisclaimer />
          <ToolCta />
          <ToolCrossLinks current="comparador" />
        </div>
      </main>
      <Footer />
      <JsonLd data={jsonLd} />
