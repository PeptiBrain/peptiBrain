...
El panel "Finanzas" muestra números ESTIMADOS; este conector trae las VENTAS REALES de Hotmart. `lib/hotmart-api.ts` (OAuth client-credentials con token cacheado — el endpoint de token de Hotmart limita fuerte, hay que pedirlo 1 vez y reutilizarlo), endpoint admin `/api/admin/hotmart-summary` (cache 10 min), tarjeta `HotmartSalesCard` en Finanzas (ingresos reales/mes, ventas, reembolsos; filtra SOLO productos PeptiBrain `8073989,8158646` para no mezclar con Confirmadísimo). Credenciales en `.env.local` (`HOTMART_CLIENT_ID/CLIENT_SECRET/BASIC_TOKEN/PRODUCT_IDS`). **Credenciales CONFIRMADAS válidas** (el token se generó bien la 1ª vez). ⚠️ **Verificación en vivo del formato de ventas PENDIENTE**: el endpoint de token bloqueó las pruebas repetidas rápidas; falta 1 prueba limpia tras el cooldown para confirmar el parseo (o se ve en el panel en producción). **Falta acción usuario**: pegar las 4 env vars de Hotmart en Vercel + redeploy.
