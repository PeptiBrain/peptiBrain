...
7. **Rate limit (60 peticiones/min por IP + bloqueo temporal 5 min) construido pero INACTIVO hasta crear la cuenta de Upstash**: crear cuenta gratis en https://upstash.com → base de datos Redis nueva → copiar `UPSTASH_REDIS_REST_URL` y `UPSTASH_REDIS_REST_TOKEN` → pegarlas en `.env.local` y en Vercel. Sin esas 2 variables, el límite simplemente no se aplica (no rompe nada, igual que Resend/Hotmart cuando faltan sus claves).
