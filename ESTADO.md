...
- **IA** ⚠️❌: infraestructura ✅ (clave en servidor, kill-switch global + límite por usuario + max_tokens, disclaimer médico). PERO **la `OPENROUTER_API_KEY` está INVÁLIDA** (probado 2026-07-21 contra el endpoint oficial de OpenRouter → "User not found" 401). El Asistente IA es una feature de Premium y ahora mismo NO responde (devuelve error a cualquier usuario Premium). **Acción del usuario**: crear una clave nueva en openrouter.ai (free tier: 50 msg/día sin tarjeta) y ponerla en `.env.local` + Vercel. Hasta entonces no se puede evaluar la calidad del output (el test de 6 preguntas —incluidas médicas que debe rechazar y anti-inyección— está listo para correr en cuanto la clave funcione).
