# ESTADO — PeptiBrain
Última actualización: 2026-07-05 | Sesión actual: 7 (testing/pulido/rigor de entrega, en progreso)

⏸️ CHECKPOINT — Última acción completada: Pasada de micro-craft (43-MICRO-CRAFT-Y-EJECUCION.md) sobre toda la app. Corregidos: (a) `tabular-nums` faltante en el mockup del hero (peso/racha), (b) comillas rectas → tipográficas en TODO el inglés (contracciones "don't"/"it's"/"you're" etc. en `messages/en.json`), (c) `text-balance` añadido a todos los `<h1>` que faltaban (onboarding, salud/péptidos/familia, building screen, términos/privacidad), (d) **bug real de UX**: varios `<input>` de Salud/Familia/Péptidos/PeptideCard usaban `text-sm` (14px) — en iOS Safari eso dispara zoom automático no deseado al enfocar el campo; se subieron todos a `text-base` (16px), (e) `min-w-0`+`truncate` en nombre de péptido (PeptideCard) y nombre/correo de familiar (Familia) — antes un nombre muy largo podía romper el layout en vez de truncarse, probado con textos extremos y confirmado que trunca bien. / Siguiente acción exacta: con esto se cierra razonablemente la Sesión 7 (testing+pulido+rigor) para lo que se puede probar sin backend real. Preguntar al usuario si pasa a Sesión 6 (servicios externos: Supabase/Hotmart/Resend/dominio) o si quiere seguir profundizando en otra cosa.
